...
const Input = () => {
  ...
  filter.addWords('fool'); // Customize the list of bad words
  ...
  const sendPost = async () => {
    ...
    if (filter.isProfane(input)) {
      console.log('Offensive content detected:', input);
      toast.error('Your post contains offensive content.');
      setLoading(false);
      return;
    }
    console.log(filter.isProfane("fool")); // Should return true
    console.log('Input:', input);
    console.log('No offensive content detected. Proceeding with post submission.');
