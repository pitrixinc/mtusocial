      ...
      <ul className="mt-4">
        {polls.map((poll) => (
          <li key={poll.id} className="bg-white shadow-md rounded-md p-4 mb-4">
            ...
            <p className="mt-4 text-lg" onClick={() => router.push(`/polls/${poll?.id}`)}>
              {poll.pollQuestion}
            </p>
            {poll?.pollImage && (
              <img className="max-h-[450px] object-cover rounded-lg mt-4" src={poll?.pollImage} alt="poll" />
            )}
            {poll?.pollVideo && (
              <video controls className="max-h-[450px] object-cover rounded-lg mt-4">
            ...
            {new Date() >= new Date(poll.endDate) ? (
              <div className="mt-4 text-red-600 font-semibold">
                <p>Poll Time Up.</p>
                <p className="mt-2 text-lg font-semibold"> Winner: {poll.totalVotes === 0 ? "No Votes" : poll.pollOptions.reduce((prev, current) => (current.votes > prev.votes ? current : prev)).text}</p>
                <p className="mt-2 text-lg font-semibold">Total Votes: {poll.totalVotes}</p>
              </div>
            ) : (
              <div className="mt-4">
